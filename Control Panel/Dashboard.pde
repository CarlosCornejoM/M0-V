...
void drawDistancePanel() {
  float x=40, y=80+240+5, w=width/2-80, h=30;
  noStroke(); fill(40,200);
  rect(x,y,w,h,8);
  textFont(fontH2); fill(200);
  textAlign(LEFT, CENTER);
  text("Ultrasonido: "+nf(distanceUS,1,2)+" cm", x+10, y+h/2);
}


void drawCubePanel() {
  float x = width/2 + 20, y = 80, w = 300, h = 200;
  float sx = 4.0 * sensorScale;
  float sy = 4.0 * sensorScale;
  float sz = 0.9 * sensorScale;

  pushMatrix();
    // Posiciona y rota el cubo
    translate(x + w/2, y + h/2, 0);
    rotateX(radians(pitch));
    rotateY(radians(roll));
    rotateZ(radians(yaw));

    // Dibuja el paralelepípedo
    noFill();
    stroke(200);
    strokeWeight(2);
    box(sx, sy, sz);

    // Vector normal escalado
    float lenCm = distanceUS;               // distancia en cm
    float extraScale = 0.01f;                // factor extra para visibilidad
    float lenPx = lenCm * 10.0f * sensorScale * extraScale;  // cm→mm→px → factor
    ...
    // Dibuja una X en la punta
    float crossSize = 8;  // tamaño de cada brazo de la X en px
    // Convertimos a coordenadas del punto final
    pushMatrix();
      translate(0, 0, z1);
      strokeWeight(2);
      // Dos líneas cruzadas
      line(-crossSize, -crossSize, 0,  crossSize,  crossSize, 0);
      line(-crossSize,  crossSize, 0,  crossSize, -crossSize, 0);
    popMatrix();
...
void drawGraphSection() {
  float x=40, y=500, w=width/2-80, h=180;
  noStroke(); fill(40,200); rect(x,y,w,h,8);
  textFont(fontH2); fill(200); textAlign(CENTER); text("Gráfico de Aceleración", x+w/2, y-10);
  stroke(120); line(x, y+h/2, x+w, y+h/2); line(x, y, x, y+h);
  noStroke(); fill(200); textFont(fontText); textAlign(CENTER);
  text("Tiempo", x+w/2, y+h+25);
  pushMatrix();
    translate(x-35, y+h/2); rotate(-HALF_PI);
    text("Valor ("+(useRaw?"LSB":"g")+")", 0,0);
  popMatrix();
...
void drawServosMotorsPanel(){
  float x=width/2+10, y=height/2 - 50, w=width/2-40, h=height-y-20;
  noStroke(); fill(40,200); rect(x,y,w,h,8);
  textFont(fontH2); fill(200); textAlign(LEFT, CENTER);
  text("Control de Servos y Motores", x+20, y+30);
}

void drawCalibrationBarFull(){
  fill(0,180); rect(0,0,width,height);
  float bw=width*0.6f, bh=30, bx=(width-bw)/2, by=(height-bh)/2;
  float t=constrain((millis()-calStart)/(float)calDur,0,1);
  noStroke(); fill(60); rect(bx,by,bw,bh,4);
  fill(0,150,250); rect(bx,by,bw*t,bh,4);
...
void setupServosMotors(){
  ...
  int y1 = height/2 + 40;
  cp5.addSlider("servo1Angle").setLabel("Servo 1")
     .setRange(0,180).setValue(90).setPosition(baseX,y1)
     .setSize(250,15).setFont(fontText);
  cp5.addSlider("servo2Angle").setLabel("Servo 2")
     .setRange(0,180).setValue(90).setPosition(baseX,y1+40)
     .setSize(250,15).setFont(fontText);
  cp5.addDropdownList("motorSelect").setLabel("Motor")
     .addItem("Motor 1",0).addItem("Motor 2",1).addItem("Ambos",2)
     .setPosition(baseX,y1+80).setSize(160,60)
     .setItemHeight(18).setBarHeight(18).setFont(fontText);
  cp5.addSlider("motorVelocity").setLabel("Potencia")
     .setRange(-255,255).setValue(0).setPosition(baseX,y1+150)
     .setSize(250,15).setFont(fontText);
  cp5.addSlider("motorTime").setLabel("Tiempo (ms)")
     .setRange(0,5000).setValue(1000).setPosition(baseX,y1+190)
     .setSize(250,15).setFont(fontText);
}
...
void serialEvent(Serial p){
  ...
  if(line.startsWith("IR raw: 0x")) {
    ...
    if(irCodes.size()>MAX_IR_CODES) irCodes.remove(0);
  }
  else if(line.startsWith("Distance:")) {
    // form: "Distance: XX.XX cm"
    String[] sp = line.split(" ");
    distanceUS = float(sp[1]);
  }
...
void motorSelect(int v){
  motorSelect=v+2; sendMotorCommand();
}

void motorVelocity(float v){
  motorVelocity=int(v); sendMotorCommand();
}
