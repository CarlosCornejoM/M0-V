/* autogenerated by Processing revision 1301 on 2025-05-13 */
...
int pitchColor = color(255, 128, 0);   // Naranja
int rollColor  = color(128, 0, 255);   // Morado
int yawColor   = color(0, 255, 255);   // Cian
...
public void drawTelemetryPanel() {
  ...
  fill(0xFFFF0000); text("X:"+ax, x+pad+100, ty+100);
  fill(0xFF00FF00); text("Y:"+ay, x+pad+200, ty+100);
  fill(0xFF0000FF); text("Z:"+az, x+pad+300, ty+100);
...
public void drawAccelerationGraph(float x, float y, float w, float h) {
  stroke(120);
  line(x, y+h, x+w, y+h);
  line(x, y,   x,   y+h);
  ...
  drawDataCurve(histAx, 0xFFFF0000, x, y, w, h);
  drawDataCurve(histAy, 0xFF00FF00, x, y, w, h);
  drawDataCurve(histAz, 0xFF0000FF, x, y, w, h);
}

public void drawDataCurve(ArrayList<Integer> data, int c, float x, float y, float w, float h) {
...
public void drawAngleBar(float x, float y, float w, float h, float val, String label, int c) {
...
public void drawCalibrationBar() {
  ...
  text("Calibrando... " + PApplet.parseInt(t*100) + "%", x + w/2, y + h/2);
  if (t >= 1) calibrating = false;
}

public void drawSerialConsole() {
