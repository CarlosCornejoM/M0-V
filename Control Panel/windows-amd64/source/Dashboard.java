/* autogenerated by Processing revision 1301 on 2025-05-16 */
...
int pitchColor, rollColor, yawColor;
...
public void drawTelemetryPanel() {
  ...
  fill(0xFFFF0000); text("X:"+ax, x+p+100,ty+100);
  fill(0xFF00FF00); text("Y:"+ay, x+p+160,ty+100);
  fill(0xFF0000FF); text("Z:"+az, x+p+220,ty+100);

  fill(235); text("Acel (g):", x+p, ty+124);
  fill(0xFFFF0000); text("X:"+nf(ax_g,1,3)+"g", x+p+100,ty+124);
  fill(0xFF00FF00); text("Y:"+nf(ay_g,1,3)+"g", x+p+160,ty+124);
  fill(0xFF0000FF); text("Z:"+nf(az_g,1,3)+"g", x+p+220,ty+124);

  fill(235);
  text("Gyro → X:"+gx+"  Y:"+gy+"  Z:"+gz, x+p, ty+156);
}

public void drawDistancePanel() {
...
public void drawCubePanel() {
  ...
  float sx = 4.0f * sensorScale, sy = 4.0f * sensorScale, sz = 0.9f * sensorScale;
  pushMatrix();
...
public void drawGraphSection() {
  ...
  popMatrix();
  float m = useRaw ? 16384 : 1.0f;
  if(showX) drawCurve(useRaw?histAx:histAxG, 0xFFFF0000, x,y,w,h,m);
  if(showY) drawCurve(useRaw?histAy:histAyG, 0xFF00FF00, x,y,w,h,m);
  if(showZ) drawCurve(useRaw?histAz:histAzG, 0xFF0000FF, x,y,w,h,m);
}

public void drawServosMotorsPanel(){
...
public void drawCalibrationBarFull(){
  ...
  text("Calibrando giroscopio... "+PApplet.parseInt(t*100)+"%", width/2, by+bh/2);
  if(t>=1) calibrating=false;
}

public void drawCurve(ArrayList<? extends Number> data, int c,
...
public void serialEvent(Serial p){
  ...
  if(line.startsWith("IR raw: 0x")) {
    ...
    distanceUS = PApplet.parseFloat(sp[1]);
  }
  ...
  if(line.startsWith("Pitch:")){
    ...
    gx = PApplet.parseInt(round(Float.parseFloat(nums.get(0))));
    gy = PApplet.parseInt(round(Float.parseFloat(nums.get(1))));
    gz = PApplet.parseInt(round(Float.parseFloat(nums.get(2))));
  } else if(line.startsWith("Temp")){
    temp = Float.parseFloat(nums.get(0));
  }
}

public void calibrar(int v){
  if(port!=null){
    port.write('C');
    calibrating = true;
    calStart = millis();
...
public void servo1Angle(float v){
  servo1Angle = PApplet.parseInt(v);
  if(port!=null) port.write("S,"+servo1Angle+","+servo2Angle+"\n");
}

public void servo2Angle(float v){
  servo2Angle = PApplet.parseInt(v);
  if(port!=null) port.write("S,"+servo1Angle+","+servo2Angle+"\n");
}

public void motorSelect(int v){
  motorSelect=v+2; sendMotorCommand();
}

public void motorVelocity(float v){
  motorVelocity=PApplet.parseInt(v); sendMotorCommand();
}

public void motorTime(float v){
  motorTime=PApplet.parseInt(v);
}
...
public void serialPort(int idx){
...
public void showX(boolean v){ showX=v; }
public void showY(boolean v){ showY=v; }
public void showZ(boolean v){ showZ=v; }
public void useRaw(boolean v){ useRaw=v; }
